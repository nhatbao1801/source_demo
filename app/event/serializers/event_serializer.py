...
from utils.get_provider_alive.get_provider_alive import get_profile_detail, get_business_level_code_detail, check_user_submited_form, get_rating_avg_form
...
class EventSerializerOut(serializers.ModelSerializer):
    ...
    def get_is_joined(self, instance):
        ...
        if self.context.get('request'):
            request = self.context.get('request')
            uid = request.GET.get('uid')
            participants = list(EventParticipant.objects.filter(event_id=instance.id, stage="JOINED").values_list('uid', flat=True))
    ...
    def get_co_host_info(self, instance):
        ...
        for host in instance.co_host.split(','):
            if host:
                profile = get_profile_detail(uid=host)
                if profile:
                    cohost.append(profile)
        return cohost
    ...
    def get_users_interested_in_info(self, instance):
        ...
        for user in instance.users_interested_in.split(","):
            profile = get_profile_detail(uid=user)
            if profile:
                users_interested.append(profile)
    ...
    def get_event_participant_info(self, instance):
        ...
        for participant in event_participant:
            profile = get_profile_detail(uid=participant)
            if profile:
